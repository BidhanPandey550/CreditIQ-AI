...
@router.get("/overview")
def overview(user: CurrentUser = Depends(require("analytics:read")),
             db: Session = Depends(get_db)) -> dict:
    org = user.org_id
    total = db.scalar(select(func.count()).select_from(LoanApplication)
                      .where(LoanApplication.organization_id == org)) or 0

    def count_status(status: LoanStatus) -> int:
        return db.scalar(select(func.count()).select_from(LoanApplication).where(
            LoanApplication.organization_id == org, LoanApplication.status == status)) or 0

    approved = count_status(LoanStatus.approved) + count_status(LoanStatus.disbursed) + \
        count_status(LoanStatus.active) + count_status(LoanStatus.closed)
    ...
    risk_rows = db.execute(
        select(RiskScore.band, func.count()).where(RiskScore.organization_id == org)
        .group_by(RiskScore.band)
    ).all()
    risk_distribution = {band: n for band, n in risk_rows}

    avg_score = db.scalar(select(func.avg(CreditScore.score))
                          .where(CreditScore.organization_id == org))

    exposure = db.scalar(select(func.coalesce(func.sum(LoanApplication.amount), 0)).where(
        LoanApplication.organization_id == org,
        LoanApplication.status.in_([LoanStatus.disbursed, LoanStatus.active]))) or 0
    ...
    return {
        ...
        "risk_distribution": {"low": risk_distribution.get("low", 0),
...
def status_breakdown(user: CurrentUser = Depends(require("analytics:read")),
                     db: Session = Depends(get_db)) -> dict:
    rows = db.execute(
