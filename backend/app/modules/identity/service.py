...
def authenticate(db: Session, email: str, password: str,
                 org_id: uuid.UUID | None) -> tuple[User, str, str]:
...
def issue_tokens(db: Session, user: User) -> tuple[User, str, str]:
    ...
    db.add(RefreshToken(user_id=user.id, jti=jti, token_hash=_hash_token(refresh),
                        expires_at=expires))
...
def refresh_tokens(db: Session, refresh_token: str) -> tuple[User, str, str]:
    ...
    if record.revoked_at is not None:
        ...
        db.query(RefreshToken).filter(RefreshToken.user_id == record.user_id,
...
def create_user(db: Session, org_id: uuid.UUID, data) -> User:
    ...
    roles = db.scalars(
        select(Role).where(Role.name.in_(data.role_names),
                           ((Role.organization_id == org_id) | (Role.organization_id.is_(None))))
    ).all()
    if not roles:
        raise NotFoundError("None of the requested roles exist")

    user = User(organization_id=org_id, email=data.email, full_name=data.full_name,
                password_hash=hash_password(data.password), branch_id=data.branch_id)
