...
def create(body: ApplicantCreate, user: CurrentUser = Depends(require("applicant:manage")),
           db: Session = Depends(get_db)) -> ApplicantOut:
...
def list_all(user: CurrentUser = Depends(require("applicant:read")),
             db: Session = Depends(get_db)) -> list[ApplicantOut]:
...
def get_one(applicant_id: uuid.UUID, user: CurrentUser = Depends(require("applicant:read")),
            db: Session = Depends(get_db)) -> ApplicantOut:
...
def financials(applicant_id: uuid.UUID,
               user: CurrentUser = Depends(require("applicant:read")),
               db: Session = Depends(get_db)) -> FinancialSummary:
...
def simulate_transactions(applicant_id: uuid.UUID,
    ...
    for t in txns:
        db.add(TransactionRecord(
            organization_id=user.org_id, applicant_id=applicant_id, source_type="wallet",
            txn_date=t["txn_date"], amount=t["amount"], description=t["description"],
            is_simulated=True,
        ))
    db.flush()
