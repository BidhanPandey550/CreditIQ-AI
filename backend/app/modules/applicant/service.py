...
def _monthly(amount: float, frequency: str) -> float:
    factor = {"monthly": 1, "weekly": 4.33, "daily": 30, "yearly": 1 / 12,
              "quarterly": 1 / 3}.get(frequency, 1)
    return float(amount) * factor


def create_applicant(db: Session, org_id: uuid.UUID, data) -> Applicant:
    applicant = Applicant(
        organization_id=org_id, branch_id=data.branch_id, full_name=data.full_name,
        date_of_birth=data.date_of_birth, gender=data.gender, phone=data.phone,
        email=data.email, address=data.address, is_self_employed=data.is_self_employed,
    )
    db.add(applicant)
    db.flush()

    if data.national_id:
        db.add(KycRecord(organization_id=org_id, applicant_id=applicant.id,
                         national_id=data.national_id, document_type="citizenship"))
    if data.employment:
        db.add(EmploymentRecord(organization_id=org_id, applicant_id=applicant.id,
    ...
    for lb in data.liabilities:
        db.add(LiabilityRecord(organization_id=org_id, applicant_id=applicant.id,
                               **lb.model_dump()))
...
def list_applicants(db: Session, org_id: uuid.UUID) -> list[Applicant]:
    return list(db.scalars(
        select(Applicant).where(Applicant.organization_id == org_id)
...
def compute_financials(db: Session, applicant_id: uuid.UUID) -> dict:
    ...
    incomes = db.scalars(select(IncomeRecord).where(
        IncomeRecord.applicant_id == applicant_id)).all()
    expenses = db.scalars(select(ExpenseRecord).where(
        ExpenseRecord.applicant_id == applicant_id)).all()
    assets = db.scalars(select(AssetRecord).where(
        AssetRecord.applicant_id == applicant_id)).all()
    liabilities = db.scalars(select(LiabilityRecord).where(
        LiabilityRecord.applicant_id == applicant_id)).all()
    existing = db.scalars(select(ExistingLoan).where(
        ExistingLoan.applicant_id == applicant_id)).all()
    txns = db.scalars(select(TransactionRecord).where(
        TransactionRecord.applicant_id == applicant_id)).all()
    ...
    monthly_debt = sum(float(x.monthly_installment or 0) for x in existing) + \
        sum(float(lb.monthly_payment or 0) for lb in liabilities)
    total_assets = sum(float(a.value) for a in assets)
    total_liabilities = sum(float(lb.outstanding_amount) for lb in liabilities) + \
        sum(float(x.outstanding_amount) for x in existing)
