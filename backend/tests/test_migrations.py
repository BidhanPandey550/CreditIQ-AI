...
SERVICING_TENANT_TABLES = import_module(
    "migrations.versions.92c61cc1938f_add_loan_servicing_ledger"
).TENANT_TABLES
...
def test_repository_has_one_migration_head() -> None:
    ...
    assert script.get_heads() == ["76f511ab91c2"]


def test_rls_migration_covers_every_tenant_table() -> None:
    assert set(TENANT_TABLES).union(API_KEY_TENANT_TABLES, SERVICING_TENANT_TABLES) == set(
        RLS_TABLES
    )
